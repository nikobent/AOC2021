...
def main():
    ...
    max_value = max(won)
    print(won)
    max_index = won.index(max_value)
    print(bingoNumbers[max_value])
    flat_card = list(item for sublist in cards[max_index] for item in sublist)
    sum_out = 0
    print(cards)
    print("oi")
    for k, num in enumerate(flat_card):
        if num in bingoNumbers[:max_value+1]:
            continue
        print(num)
        sum_out = sum_out + num
    print(sum_out)
    print(sum_out*bingoNumbers[max_value])
